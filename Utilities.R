...
#function to retrieve dataframe solely for product in question
...
generate.linear_model <- function(product.data){
    ...
    product.linear_model <- lm(product.data[[2]] ~ product.data[[1]]) #creating model
    print(summary(product.linear_model))

    plot <- ggplot(product.data, aes(x = months, y = product.price)) + #plotting model
        geom_point() +
...
generate.quadratic_model <- function(product.data){
    calculate.correlation_estimators(product.data)
    product.data["month_squared"] <- product.data[1]^2 #calculating x squared

    #fit quadratic regression model
    product.quadratic_model <- lm(product.data[[2]] ~ product.data[[1]] + product.data[[3]]) #creating model
...
generate.generalized_model <- function(product.data){
    ...
    product.generalized_model <- glm(product.data[[2]] ~ product.data[[1]],
            family = gaussian(link = "identity"), data = product.data) #creating model
